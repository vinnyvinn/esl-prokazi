...
class Legal extends Pre_loader
{
    ...
    private function _make_row($datas, $key=0) {
        $optoins = "";
        if(get_array_value($this->login_user->permissions,'can_edit_legal_documents') == 1 || get_array_value($this->login_user->user_permissions,'can_edit_legal_documents') == 1) {
        ...
        $responsible = $this->Users_model->get_one($datas->user_responsible);
          ...
          if(get_array_value($this->login_user->permissions,'can_delete_legal_documents') == 1 || get_array_value($this->login_user->user_permissions,'can_delete_legal_documents') == 1) {
...
public function download_legal_document_files($id) {

    $files = $this->LegalDocumentsModel->get_detail($id)['file_document'];
    download_app_files(get_setting("legal_file_path"), $files);
    ...
    function view($id){
            ...
            $model_info['documents'] = $this->LegalDocumentsModel->get_detail($id);
            $this->template->rander("legal/documents/view", $model_info);
    ...
    function save(){
        ...
        $target_path = get_setting("legal_file_path");
    ...
    private function _make_reminders_row($datas, $key=0) {
        $optoins = "";
        if(get_array_value($this->login_user->permissions,'can_edit_legal_reminders') == 1 || get_array_value($this->login_user->user_permissions,'can_edit_legal_reminders') == 1) {
        ...
        if(get_array_value($this->login_user->permissions,'can_delete_legal_reminders') == 1 || get_array_value($this->login_user->user_permissions,'can_delete_legal_reminders') == 1) {
            $optoins .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_project'), "class" => "delete", "data-id" => $datas->id,
                "data-action-url" => get_uri("legal/delete_reminders"), "data-action" => "delete"));
        }
