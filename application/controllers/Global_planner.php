...
class Global_planner extends Pre_loader {
    ...
    function modal_form() {
        ...
        $this->load->view('planner/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($id && !$this->login_user->is_admin) {
            $event_info = $this->Global_planner_model->get_one($id);
            if ($event_info->created_by != $this->login_user->id) {
                redirect("forbidden");
            }
        }

        $save_id = $this->Global_planner_model->save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->Global_planner_model->delete($id)) {
    ...
    function calendar_events() {
        ...
        $list_data = $this->Global_planner_model->get_details($options)->result();
        $result = array();
    ...
    function view() {
        ...
        $model_info = $this->Global_planner_model->get_details(array("id" => $event_id))->row();
        ...
        if ($model_info->id) {
            ...
            $this->load->view('planner/view', $view_data);
        } else {
            show_404();
        }
    }
