...
class Preventive extends Pre_loader
{
  ...
  {
      $km = $this->db->query("SELECT assets.job_km_readings FROM assets 
      LEFT JOIN  jobs ON jobs.vehicle_no=assets.id WHERE assets.id=$id ORDER BY jobs.vehicle_no DESC")->row()->job_km_readings;
      echo json_encode($km);

  }
  public function miles_reading($id)
  {
    $miles = $this->db->query("SELECT assets.job_miles_readings FROM assets 
      LEFT JOIN  jobs ON jobs.vehicle_no=assets.id WHERE assets.id=$id ORDER BY jobs.vehicle_no DESC")->row()->job_miles_readings;
      echo json_encode($miles);
  }
  public function machine_hours($id)
  {
    $hrs = $this->db->query("SELECT assets.job_machine_hours FROM assets 
      LEFT JOIN  jobs ON jobs.vehicle_no=assets.id WHERE assets.id=$id ORDER BY jobs.vehicle_no DESC")->row()->job_machine_hours;
  ...
  public function modify_job($id){
      ...
      $view_data ['job_types_dropdown'] = $this->Job_types_model->get_all_where(array("deleted" => 0))->result();
      $view_data['fuel_dropdown'] = $this->Fuel_balances_model->get_all_where(array("deleted" => 0))->result();
      $this->template->rander('maintenance/preventive/modify_form', $view_data);
  }
  ...
  if(!empty($partArr)){
   ...
   $data = array(
     ...
     $this->db->where('id', $this->input->post('id'))->update('jobs', $data);
  ...
  public function inventory_list(){
        ...
        $view_data['inventory_listing'] = $this->db->query("SELECT * FROM assets")->result();

        $this->template->rander('maintenance/preventive/inventory_list',$view_data);
  ...
  public function print_inventory(){
      ...
      $this->pdf2->load_view('maintenance/preventive/inventory_list_print', $view_data);
      $this->pdf2->render();
    ...
    public function extract_inventory()
    {
        ...
        $table_columns = array("Code", "Description");
        ...
        $employee_data =$this->db->query("SELECT * FROM assets")->result();
        ...
        header('Content-Disposition: attachment;filename="Inventory List Data.xls"');
...
$mileage=array("job_km_readings" => $model['km_reading'],"job_miles_readings" => $model['miles_reading'],
  "job_machine_hours" => $model['hours'],"next_time_km" => $model['next_time_km'],"next_time_miles" => $model['next_time_miles'],"next_time_hours" => $model['next_time_hours'],"next_service_date" => $model['next_service_date']);
...
public function print_job($id)
{
  ...
  $view_data['reports_data']=$this->db->query("SELECT jobs.*,assets.code as reg,assets.description as make,employees.name as employee,
    jobs.description as description,assets.km_reading as km_reading,assets.next_time_km,assets.next_time_miles,
    jobs.created_at as created,parts_suppliers.name as supplier,spares.stock_name,
   spares.qnty_out,spares.total as totalcost FROM jobs
