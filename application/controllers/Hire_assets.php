...
class Hire_assets extends Pre_loader
{
    ...
    public function add_asset()
    {
        ...
        if ($this->input->post('asset_inhouse')) {
            ...
            $existing = $this->input->post('asset_inhouse');

            $SAGEQuery = "SELECT cAssetCode,cAssetDesc, ubFAForHire,ulFAHireItemGroup,idAssetNo,fPurchaseValue FROM _btblFAAsset
      WHERE  idAssetNo IN ( '" . implode("', '", $existing) . "' )";
            $sage = $this->SAGE_DB()->query($SAGEQuery)->result();

            foreach ($sage as $items) {
                $sage_data[] = $items->cAssetDesc;
            }
        ...
        $output = array_map(function ($sage_data) {
            return $sage_data;
        }, $sage_data);
        ...
        $data = array(
            ...
            'asset' => $output ? implode(', ', $output) : implode(', ', $asset),
        ...
        if ($client_id = $query->client_id) {
            $match_client = $this->db->query("SELECT * FROM sage_clients WHERE id=$client_id")->row()->name;
...
////            'iWarehouseID',
    ...
    private function _make_item_row($data)
    {
        ...
        return array(
            $data->equipment ? $data->equipment : $data->asset,
