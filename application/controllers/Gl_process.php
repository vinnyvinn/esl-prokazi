...
class Gl_process extends Pre_loader
{
   ...
   public function save_def(){
    ...
    $Posting_Date = $this->input->post('post_date');
     ...
     $proj_code=$this->db->query("SELECT code,project_link FROM sage_projects WHERE project_link=$Prj")->row()->code;
   ...
   if(!$this->checkDefferal($proj_code)){
    ...
    $this->SAGE_DB()->query("Exec ESL_DEF_PROJ '$proj_code','$Income_Contra' , $Expense_Contra , '$Posting_Date'");
 ...
 public function checkDefferal($code){
    ...
    return $this->SAGE_DB()->query("Select Count (*) from PostGL G Inner Join Project P on G . Project = P . ProjectLink
