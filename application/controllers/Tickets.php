...
{
 $id = $this->input->post('thirdparty_users_dropdown');
 $user = $this->db->query("SELECT * FROM tbl_third_party WHERE id=$id")->row_array();
 $data = array('message' => $this->input->post('message'),
 ...
 $parser_data["USER_NAME"] = $email_data['sender_name'];
 $parser_data["MESSAGE"] =  $email_data['message'];
 $parser_data["THIRD_PARTY_NAME"] =  $email_data['username'];
 $parser_data["USER_EMAIL"] =  $email_data['sender_email'];
 $parser_data["TICKET_URL"] =  $email_data['ticket_url'];
 $parser_data["SIGNATURE"] =  $email_template->signature;

 $message = $this->parser->parse_string($email_template->message, $parser_data, TRUE);

 if (send_app_mail($email_data['email'], $email_template->subject, $message)) {
...
function ticket_solved_email($ticket_id){
 ...
 LEFT JOIN users ON users.id=support_entries.created_by
  ...
  $parser_data["TICKET_ID"] = $entries->id;
  $parser_data["CREATED_BY"] = $entries->username;
  $parser_data["CREATED_AT"] =  $entries->created_at;
  $parser_data["SOLVED_BY"] =  $this->login_user->first_name .' '. $this->login_user->last_name ;
  $parser_data["SIGNATURE"] =  $email_template->signature;

  $message = $this->parser->parse_string($email_template->message, $parser_data, TRUE);
  send_app_mail($entries->email, $email_template->subject, $message);
  ...
  if ($data->status === "new") {
    ...
    // load ticket details view
        ...
        public function view_support($ticket_id = 0)
        {
          ...
          $options = array("id" => $ticket_id);
         // $options["access_type"] = $this->access_type;
            $ticket_info=$this->Support_entries_model->get_one($ticket_id);
        ...
        public function save_support() {
          ...
          if ($comment_id) {
            ...
            $view_data['comment'] =  $this->Support_comments_model->view_comment($support_options);
            $comment_view = $this->load->view("tickets/support_row", $view_data,true);
        ...
        public function download_support_files($id) {
