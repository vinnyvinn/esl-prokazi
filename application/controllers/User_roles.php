...
class User_roles extends Pre_loader
{
    ...
    {
        $view_data['users_dropdown'] = $this->Users_model->get_all_where(array("deleted" => 0))->result();
    $this->template->rander('roles/user_form',$view_data);
    ...
    function permissions($role_id)
         {
             ...
             if ($role_id) {
            $view_data['model_info'] = $this->Users_model->get_one($role_id);

            $permissions = unserialize($view_data['model_info']->user_permissions);
            ...
            $this->template->rander("roles/user_permissions", $view_data);
        }
    }
    ...
    function save_permissions()
    {
        ...
        $save_id = $this->Users_model->save($data, $id);
        return redirect(base_url('user_roles/permissions/'.$id));
    }
}
