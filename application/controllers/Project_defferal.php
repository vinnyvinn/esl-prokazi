...
public function add_finance(){
  ...
  $project_id=$this->input->post('project_id');
  $fromSage = $this->SAGE_DB()->query("SELECT ProjectCode,ProjectLink FROM Project WHERE ProjectLink = $project_id")->row_array();
  $proj_code = $fromSage['ProjectLink'];
  ...
  if(!empty($this->db->query("SELECT * FROM defferals WHERE project_id=$project_id")->row_array())){
   ...
   if($this->input->post('finance_sign_off')==1){
     ...
     $this->SAGE_DB()->query("UPDATE Project SET ActiveProject = 0 WHERE ProjectLink =$proj_code");
    $this->db->query("UPDATE sage_projects SET status = 0 WHERE project_link =$proj_code");
  }
