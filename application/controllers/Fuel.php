...
public function dateDiffTs($start_ts, $end_ts) {
  ...
  $variables = $this->db->query("SELECT assets.code,assets.next_time_km,assets.next_time_miles,assets.next_time_hours,assets.next_service_date,
    employees.* FROM assets
  LEFT JOIN employees ON employees.id= assets.driver_id WHERE assets.id=$machine_no")->row_array();
   ...
   $next_date = $this->dateDiffTs(time(),strtotime($variables['next_service_date']));
      
//    if(($next_km >=0 && $next_km <= 500 ) || $next_date < 1 ){
//
//      $this->fuel_mail($variables);
//     }
//     if (($next_mile >=0 && $next_mile <=300) || $next_date < 1) {
//
//      $this->fuel_mail($variables);
//     }
//     if(($next_mile >=0 && $next_hr <=50) || $next_date < 1) {
//        $this->fuel_mail($variables);
//     }
//
...
//   if($next_km >0 && $next_km <= 500){
//      $this->fuel_mail($variables);
//     }
//     elseif ($next_mile >0 && $next_mile <=300) {
//      $this->fuel_mail($variables);
//     }
//     elseif($next_hr >0 && $next_hr <=50){
//     $this->fuel_mail($variables);
//     }
}
...
public function fuel_mail($variables) {
  ...
  $parser_data["MILEAGE"] = $variables['next_time_km'] ? $variables['next_time_km'] .' km' :
  ($variables['next_time_miles'] ? $variables['next_time_miles'] .' mi' : $variables['next_time_hours'] .' hrs');
