...
class Lawsuits extends Pre_loader
{
    ...
    function list_data()
    {
       // $list_data = $this->LegalCasesModel->getCases(0);
        $list_d = $this->db->query("SELECT tbl_cases.*,tbl_case_procedures.name as steps,tbl_case_type.name as case_type FROM tbl_cases
                 LEFT JOIN tbl_case_procedures ON tbl_case_procedures.id=tbl_cases.case_procedure
                 LEFT JOIN  tbl_case_type ON tbl_case_type.id=tbl_cases.case_type where tbl_cases.deleted=0")->result();

        $result = [];
        foreach ($list_d as $key => $data) {
            $result[] = $this->_make_row2($data, $key);
        }
        echo json_encode(array("data" => $result));
    }

    //make table with action buttons
    private function _make_row2($datas, $key)
    {
        $optoins = "";

        $case_type = $this->LegalCaseTypesModel->get_one($datas->case_type);
        $procedure = $this->LegalCasesModel->get_one($datas->case_procedure);

        $name = anchor(get_uri("lawsuits/view/" . $datas->id), $datas->name);



            $optoins .= modal_anchor(get_uri("lawsuits/cases_modal"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_case'), "data-post-id" => $datas->id));


            $optoins .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_case'), "class" => "delete", "data-id" => $datas->id,
                "data-action-url" => get_uri("lawsuits/deletecase"), "data-action" => "delete"));
        ...
        return array(
            (int)$key + 1,
            $name,
            (empty($datas->case_type))?'Not-Set':$datas->case_type,
    ...
    function cases_modal()
    {

        $id = $this->input->post('id');

        $view_data['model_info'] = $this->LegalCasesModel->get_one($id);
        $this->load->view("legal/lawsuits/cases_modal", $view_data);
    ...
    function updatecases(){
        ...
        $saved = $this->db->update('tbl_cases',$data);

        echo json_encode(array("success" => true, 'message' => (!$saved) ? "Case could not be closed" : "Case closed"));
    ...
    function type_modal()
    {
        ...
        $this->load->view("legal/types/type_form", $view_data);
    }
    ...
    function deletecase()
    {
        ...
        $deleted = false;
        if ($id) {
            $this->LegalCasesModel->row_delete($id);
            $deleted = true;
        }
        echo json_encode(array("success" => $deleted, 'message' => (!$deleted) ? lang('record_cannot_be_deleted') : lang('record_deleted')));
    ...
    function types_list_data()
    {
        ...
        foreach ($active as $key => $data) {
            ...
            $result[] = $this->_make_types_row($data, $key, ($key == 0) ? true : false);
    ...
    private function _make_types_row($datas, $key, $disabledelete = false)
    {

        $optoins = "";
        if ($this->can_edit_types()) {
            $optoins .= modal_anchor(get_uri("lawsuits/type_modal"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_type'), "data-post-id" => $datas->id));
        }

        if ($this->can_delete_types() && !$disabledelete) {
            $optoins .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_type'), "class" => "delete", "data-id" => $datas->id,
                "data-action-url" => get_uri("lawsuits/deletetype"), "data-action" => "delete"));
        }
    ...
    private function _make_type_row($datas, $key, $disabledelete = false)
    {
        $optoins = "";
        if ($this->can_edit_types()) {
            $optoins .= modal_anchor(get_uri("lawsuits/type_modal"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_legal_document'), "data-post-id" => $datas->id));
        }
