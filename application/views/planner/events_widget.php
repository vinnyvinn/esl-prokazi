            ...
            <div style="min-height: 190px;">
                ...
                if ($events) {
                    foreach ($events as $event) {
                        ?>
                        <div class="mb20">
                            <div><?php echo modal_anchor(get_uri("global_planner/view"), "<i style='color:" . $event->color . "' class='fa " . get_event_icon($event->share_with) . "'></i></span> " . $event->title, array("data-post-id" => encode_id($event->id, "event_id"), "title" => lang("event_details"))); ?></div>
                            <div><?php $this->load->view("global_planner/event_time", array("model_info" => $event)); ?></div>
                        </div>
                        <?php
                    }
                } else {
